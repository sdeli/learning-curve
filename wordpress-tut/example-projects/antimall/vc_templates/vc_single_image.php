...
switch ( $onclick ) {
	...
	case 'category':
		...
		if( $img_category_id != 'All items' ) {

			$term = get_term_by( 'id', $img_category_id, 'product_cat' );
			...
			$link = site_url( '/shop');
			$cat_name = esc_html__('All items collection', 'antimall');
		}		
}
...
if ( $link ) {
	...
	$cat_name_html = '';
	if( isset( $onclick_category )  && $onclick_category ) {
		$cat_name_html = '<span class="img-cat-name">' . $cat_name . '</span>';
	}

	$html = '<a ' . vc_stringify_attributes( $a_attrs ) . ' class="' . $wrapperClass . '">' . $img['thumbnail'] . $cat_name_html . '</a>';
